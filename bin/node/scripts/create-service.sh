...
TESTNET="ajuna_aj1_raw.json"

REPO_DIR="/home/$USER/substrate"
VALIDATOR_DIR="$REPO_DIR/tmp/validator-chain"
FULLNODE_DIR="$REPO_DIR/tmp/fullnode-chain"
CHAIN_SPEC="$REPO_DIR/bin/node/cli/res/$TESTNET"
NODE_EXE="$REPO_DIR/target/release/substrate"

### FUNCTIONS ###
createValidatorDaemonService() {
    sudo bash -c 'cat > /etc/systemd/system/substrate-validator.service <<EOF
[Unit]
Description=Ajuna Network Validator

[Service]
WorkingDirectory='$REPO_DIR'

ExecStart='$NODE_EXE' --base-path '$VALIDATOR_DIR' --chain='$CHAIN_SPEC' --port 30333 --ws-port 9944 --rpc-port 9933 --validator --rpc-methods=Unsafe --name "'$NODE_NAME'" --rpc-cors all
Restart=always
RestartSec=120
...
[Unit]
Description=Ajuna Network Full Node

[Service]
WorkingDirectory='$REPO_DIR'

ExecStart='$NODE_EXE' --base-path '$FULLNODE_DIR' --chain='$CHAIN_SPEC' --port 30333 --ws-port 9944 --rpc-port 9933  --name "'$NODE_NAME'"
Restart=always
RestartSec=120
